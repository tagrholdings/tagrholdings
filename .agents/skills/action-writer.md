...
- Errors thrown in the Service (`throw new Error(...)`) are caught by `safe-action` and automatically become `{ success: false, error: "message" }` — the Action doesn't need a manual `try/catch` around the Service call.
