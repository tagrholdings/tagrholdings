...
- `badges` (the `{ [navItem.badgeKey]: count }` map, e.g. `leadsInboxUnread`) is a `HubChrome`/`Sidebar`/`BottomNav` concern now, not a per-page prop — compute it in `app/(hub)/layout.tsx` once leads-inbox unread counts exist, not in individual pages.
...
## Boards: Leads Inbox, Projects, Activities
...
- **`/leads-inbox`** renders the one system board (`isSystem: true`, "Leads"), seeded lazily per tenant by `pipelineService.ensureDefaultBoard()` (called in `leads-inbox/page.tsx`). Default columns in `DEFAULT_BOARD_COLUMNS` (`pipeline.constants.ts`). The system board can't be renamed, archived or deleted — `pipelineService.getEditableBoard()` enforces it.
...
**Shared components** live in `components/pipeline/` (not a route's `_components/`), because Leads Inbox and Projects render the exact same view: `PipelineView` (toolbar + board/list + detail; optimistic add + stage move), `PipelineBoard`, `PipelineCard`, `PipelineListView` (detailed table: title, organization, contact, stage, activities done/total), `PipelineItemDetail`, `PipelineItemActivities`, `CreatePipelineItemVault`, `ActivityProgressBadge` ("4/8"), and `links.ts`'s `pipelineItemHref()` (leads → `/leads-inbox?item=`, project items → `/pipeline?board=&item=`).
...
**Deep links**: `/leads-inbox?item=`, `/pipeline?board=&item=`, `/activities?activity=`, `/contacts?contact=` pre-open the matching detail panel (read in each `page.tsx` from `searchParams`, passed down as `initialSelectedId`).
