...
- The screens share `LeadsInboxTabs` (real links, one route each; four now: Inbox, Search profiles, Email sources, Engine spend). Everything under `/leads-inbox/*` lights up the one "Leads Inbox" nav item via `isNavItemActive`'s prefix match.
