...
## Motion library — framer-motion by default, GSAP is a scoped exception

`framer-motion` is the default animation library across the app (`lib/animations.ts`'s variants, the bottom-nav active pill, `Toaster`) — don't add a second animation dependency for a component that fits framer-motion's declarative/layout-animation model.

`app/auth/sign-in/_components/SignInCard.tsx` is a deliberate, scoped exception: it uses `gsap` + `@gsap/react`'s `useGSAP` for the entrance timeline (logo → wordmark → card → staggered form fields), because that's what was asked for that page specifically. Don't default to GSAP elsewhere just because it's now a dependency — new hub screens still reach for framer-motion first, per the rest of this file.
...
- **Main content area**: page background (`--background`), generous padding on desktop, tighter edge padding on mobile (16px), a `.label-kicker` section label at the top (e.g. "DEAL FLOW") — this is `AppHeader`'s `kicker`, not a second label repeated in the page body.
