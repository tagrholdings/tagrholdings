...
3. **On `www.tagrholdings.com`** (or any other domain pointed at this deployment): any path *not* on the marketing allowlist redirects to the same path on `crm.tagrholdings.com`. This stops someone from reaching CRM screens through the marketing domain.
...
- **Local dev has no real subdomain.** `proxy.ts` also matches `crm.localhost` so `http://crm.localhost:3000` behaves like production during `next dev` — but `crm.localhost` needs to resolve to `127.0.0.1`, which some OS/browser combinations do out of the box and others need a `hosts` file entry for. `localhost:3000` alone (no subdomain) falls through to the marketing-host branch.
