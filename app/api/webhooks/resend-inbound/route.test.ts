...
vi.mock("@/modules/leads/leads.repository", () => ({ leadsRepository: { findIdByDedupeKey: vi.fn(), createIfNew: vi.fn() } }));
vi.mock("@/modules/lead-extraction/lead-extraction.service", async (importActual) => ({
  ...
  leadExtractionService: { extract: vi.fn() },
}));
...
beforeEach(() => {
  ...
  vi.mocked(leadExtractionService.extract).mockResolvedValue({
    fields: { businessName: "Profitable HVAC company", askingPrice: "$850,000" },
    usage: { provider: "openai", operation: "extract", model: "gpt-4o-mini", inputTokens: 900, outputTokens: 120, costUsd: 0.0002 },
    fallback: false,
  });
...
describe("resend-inbound webhook — a valid email.received", () => {
  it("fetches the full body, extracts it, and saves an email_digest lead", async () => {
    ...
    expect(vi.mocked(leadExtractionService.extract).mock.calls[0][0]).toMatchObject({ sourceType: "email_digest" });
    expect(vi.mocked(leadExtractionService.extract).mock.calls[0][0].text).toContain("Owner retiring");

    expect(leadsRepository.createIfNew).toHaveBeenCalledWith(
      TENANT,
      expect.objectContaining({
        sourceType: "email_digest",
        sourceUrl: "https://bizlistings.test", // the sender's site
        dedupeKey: `email:${EMAIL_ID}`,
  ...
  it("is idempotent: a redelivered webhook for the same email creates nothing and pays for nothing", async () => {
    vi.mocked(leadsRepository.findIdByDedupeKey).mockResolvedValue("lead-1");
    const res = await POST(signedRequest(receivedEvent));
    expect(await res.json()).toEqual({ received: true, outcome: "lead" });
