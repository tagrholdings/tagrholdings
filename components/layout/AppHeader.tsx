...
function DesktopHeader({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-20 h-16 shrink-0 items-center gap-4 border-b border-divider bg-background px-6",
        className
      )}
    >
...
export function AppHeader(props: AppHeaderProps) {
  const { forceMode, className } = props;

  if (forceMode === "mobile") {
    return <MobileHeader {...props} className={cn("flex", className)} />;
  }
  if (forceMode === "desktop") {
    return <DesktopHeader {...props} className={cn("flex", className)} />;
  }

  return (
    <>
      <MobileHeader {...props} className={cn("flex md:hidden", className)} />
      <DesktopHeader {...props} className={cn("hidden md:flex", className)} />
    </>
  );
}
