...
interface NavBarProps {
  links: NavLink[];
  /**
   * Replaces the default "Contact Us" link (which scrolls to the landing
   * page's `#contact` section — there is no such section on the portal).
   */
  contact?: ReactNode;
}

/** Shared by the default link and any `contact` slot, so they look identical. */
export const navContactClassName =
  "rounded-sm border border-[var(--ink)] px-3 py-1.5 md:px-4 md:py-2 font-mono text-[10px] md:text-[12px] uppercase tracking-[0.04em] transition hover:bg-[var(--ink)] hover:text-[var(--cream)] text-center";

export function NavBar({ links, contact }: NavBarProps) {
  return (
    <nav className="sticky top-0 z-50 border-b border-[rgba(27,29,31,0.14)] bg-[rgba(237,232,223,0.92)] backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-6 md:py-5 lg:px-8">
        ...
        {contact ?? (
          <Link href="#contact" className={navContactClassName}>
            Contact Us
          </Link>
        )}
      </div>
